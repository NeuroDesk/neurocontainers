...
docker build . --file ${IMAGENAME}.Dockerfile --security-opt seccomp=unconfined --tag $IMAGEID:$SHORT_SHA --cache-from $IMAGEID --label "GITHUB_REPOSITORY=$GITHUB_REPOSITORY" --label "GITHUB_SHA=$GITHUB_SHA"
